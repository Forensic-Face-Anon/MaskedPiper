...
mypath = "./driving/" #this is your folder with (all) your video(s)
...
outputf_mask = "./Output_Experiment/"
...
for vidf in vfiles:
    ...
    videoname = vidf
    videoloc = os.path.join(mypath, videoname)
        ...
        while (True):
            ...
            if ret == True: #if there is a frame
                ...
                if  np.all(results.segmentation_mask) != None: #check if there is a pose found
                    ...
                    samplebody.insert(0, time_var)
                    samplehands.insert(0, time_var)
                    sampleface.insert(0, time_var)
                ...
                time_var = time_var+(1000/samplerate)#update the time variable  for the next frame
    ...
    print(f"[Timing] Total processing time: {elapsed:.2f} seconds")
    
     ####################################################### data to be written row-wise in csv fil
        
    # But we do not need this for this project
